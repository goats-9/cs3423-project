...
\lstinputlisting{codes/example.tblt}
...
The syntax for declaration statements is given in . Note that Tabulate allows
for multiple declarations in a single statement for the same data type, with the
help of comma for separation.

\subsubsection{Function Calls}
The syntax for function calls is given in . Note that nested function calls are
allowed in Tabulate. Further, note that there are no return statements in
Tabulate as the variable and its type are declared at function declaration.
...
Tabulate supports the jump statements \texttt{break} and \texttt{continue}.
...
can be nested in other compound statements.
...
The syntax for definition of a function is given in . Note that the return
variable need not be declared in the function body.
